...
RegisterNUICallback('GetGoogleMapsMaps', function(data, cb)
    cb(Config.GoogleMaps)
end)
...
RegisterNUICallback('CasinoPhoneGoogleMaps', function(data)
    if data.action == 1 then
        TriggerServerEvent('qb-phone:server:SetGoogleMapsMaps', data)
    elseif data.action == 2 then
        SetNewWaypoint(data.x, data.y)
        QBCore.Functions.Notify('GPS set', "success")
    end
